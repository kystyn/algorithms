  ...
  // up is parent
    ...
    // this is not the leaf
      ...
      // toDelete is only child
        ...
        toDelete->up->down->up = toDelete->up;
        toDelete->down->up = toDelete->down;
        // if toDelete has two children - meld it
        if (toDelete->down->up != toDelete) {
          heap h0 = bottomUpMeld(findMajorPath(toDelete->down), findMajorPath(toDelete->down->up));
          toDelete->up->down = bottomUpMeld(h0.majorPath, findMajorPath(toDelete->up->down));
        }
        else {
          toDelete->up->down = bottomUpMeld(h0.majorPath, findMajorPath(toDelete->up->down));
        }
